...
static char cleavage_code(char *seq, double *predictions,
			  int offset, double cutoff, int *cleavage_sites)
{
				 ...
				 const char *name)
{
	fprintf(file, "--------------------------------------\n\n");
	fprintf(file,
		"Number of cleavage sites %d. Number of amino acids %d. Protein name %s\n\n",
				       ...
				       double *predictions)
{
	...
	if (remaining_positions) {
		...
		for (i = seq_len - remaining_positions; i < seq_len - 1; i++) {
			fprintf(nc->file, "%c",
				cleavage_code(seq, predictions, i,
					      co->cutoff, &cleavage_sites));
		}
		fprintf(nc->file, "\n");
	}
				 ...
				 char *seq, int seq_len, double *predictions)
{
	...
	for (i = 0; i < seq_len; i++) {
		fprintf(nc->file, "%4d   %c  %c    %f %s\n", i + 1, seq[i],
			cleavage_code(seq, predictions, i, co->cutoff,
				      &cleavage_sites), predictions[i], name);
	}
	write_netchop_footer(nc->file, cleavage_sites, seq_len, name);
}
					  ...
					  double cutoff, int short_format)
{
	...
	if (nc->file == NULL) {
		fprintf(stderr,
			"Unable to open file '%s' for writing. Exiting.\n",
			output_path);
		exit(EXIT_FAILURE);
	}
