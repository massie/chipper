...
static char *test_pos_neg_samples()
{
	...
	struct feature_node positive_features[GENERATED_SAMPLE_LEN *
					      NUM_AA_PROPERTIES + 1];
	struct feature_node negative_features[GENERATED_SAMPLE_LEN *
					      NUM_AA_PROPERTIES + 1];
	...
	peptide_to_features(positive_sample, GENERATED_SAMPLE_LEN,
			    positive_features, ARRAY_LEN(positive_features));
	peptide_to_features(negative_sample, GENERATED_SAMPLE_LEN,
			    negative_features, ARRAY_LEN(negative_features));

	predict_probability(data_model, positive_features,
			    probabilities_true_false);
	positive_probability = probabilities_true_false[0];
	predict_probability(data_model, negative_features,
			    probabilities_true_false);
