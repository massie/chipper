...
void peptide_to_features(const char *peptide, int peptide_len,
                         struct feature_node *features, int features_len) {
  int i, rval;
  assert(features_len == peptide_len * NUM_AA_PROPERTIES + 1);
  for (i = 0; i < peptide_len; i++) {
    rval = get_aa_properties(peptide[i], features + i * NUM_AA_PROPERTIES);
    if (rval == 0) {
...
struct chipper_output *new_chipper_output(const char *filename,
  ...
  switch (format) {
  case FASTQ_OUTPUT:
    return new_fastq_output(filename, output_probabilities, active_cutoff);
  case NETCHOP_OUTPUT:
...
int predict_cleavage(const char *fasta_filename, const char *output_filename,
                     chipper_output_format output_format,
  ...
  if (predictions == NULL) {
    fprintf(stderr, "Unable to allocate memory for predictions. Exiting.\n");
    exit(EXIT_FAILURE);
  }
  ...
  output =
      new_chipper_output(output_filename, output_format, is_probability_model,
                         output_probabilities, cutoff_provided, cutoff);
  ...
  while ((len = kseq_read(seq)) >= 0) {
    ...
    if (predictions_len < seq->seq.l) {
      predictions = realloc(predictions, seq->seq.l * sizeof(double));
      if (predictions == NULL) {
        fprintf(stderr,
    ...
    for (sample_index = GENERATED_SAMPLE_LEN; sample_index <= seq->seq.l;
         sample_index++) {
      peptide_to_features(seq->seq.s + sample_index - GENERATED_SAMPLE_LEN,
                          GENERATED_SAMPLE_LEN, features, ARRAY_LEN(features));
      cleavage_site = sample_index - GENERATED_SAMPLE_LEN / 2 - 1;
      if (is_probability_model) {
        predict_probability(data_model, features, probabilities_true_false);
        predictions[cleavage_site] = probabilities_true_false[0];
      } else {
        predictions[cleavage_site] = predict(data_model, features);
      }
    }

    output->write_record_cb(output, seq->name.s, seq->name.l, seq->comment.s,
                            seq->comment.l, seq->seq.s, seq->seq.l,
                            predictions);
  }
