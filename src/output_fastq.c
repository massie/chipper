...
static void fastq_write_record(struct chipper_output *co, char *name,
  ...
  for (i = 0; i < seq_len; i++) {
    if (fq->output_probabilities) {
      CHECK(gzprintf(fq->file, "%c", prediction_to_char(predictions[i])));
    } else {
      CHECK(gzprintf(fq->file, "%c",
                     isnan(predictions[i])
                         ? '!'
...
struct chipper_output *new_fastq_output(const char *output_path,
                                        int output_probabilities,
                                        double cutoff) {
  struct chipper_fastq_output *fq = malloc(sizeof(struct chipper_fastq_output));
  if (fq == NULL) {
